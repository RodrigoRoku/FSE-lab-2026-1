...
port = 80
controladorDelGpio = gpioManager.gpioManager() 
...
def main():
	...
	try:
		...
		controladorDelGpio.continueFunctionLoop = False 
	except:
		print(sys.exc_info())
	finally:
		controladorDelGpio.stop()
...
if __name__ == "__main__":
	webserverThread = threading.Thread(target=main)
	gpioManagerThread = threading.Thread(target=controladorDelGpio.mainLoop)

	webserverThread.start()
	gpioManagerThread.start()

	webserverThread.join()
	gpioManagerThread.join()
